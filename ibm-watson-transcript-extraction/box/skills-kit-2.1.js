...
function pollRepresentationInfo(client, infoURL) {
    ...
    return client.get(infoURL).then((response) => {
        ...
        switch (info.status.state) {
            ...
            case 'none':
            case 'pending':
                return delay(1000).then(() => pollRepresentationInfo(client, infoURL));
            default:
...
FilesReader.prototype.getContentStream = function getContentStream() {
    ...
    return this.fileReadClient.files.getReadStream(this.fileId, null).then((stream) => stream.pipe(bufferStream));
};
...
FilesReader.prototype.getBasicFormatContentStream = function getBasicFormatContentStream() {
    ...
    const bufferStream = new CustomStream.PassThrough();
    return this.fileReadClient.files
        .getRepresentationContent(this.fileId, FileType[this.fileType].representationType, null)
        .then((stream) => stream.pipe(bufferStream))
        .catch((e) => {
            if (e.statusCode === 401) {
                throw new TypeError(
...
SkillsWriter.prototype.createMetadataCard = function createMetadataCard(
    ...
    const template = { skill_card_type: type, status, entries };
    template.created_at = new Date().toISOString();
    template.type = SKILLS_METADATA_CARD_TYPE; // tyoe: skill_card
    template.skill = { type: SKILLS_SERVICE_TYPE, id: this.skillId }; // skill.type: service
    template.skill_card_title = { code: `skills_${title.toLowerCase()}`.replace(' ', '_'), message: title };
    template.invocation = { type: SKILLS_METADATA_INVOCATION_TYPE, id: this.requestId }; // invocation.type: skill_invocation
    if (optionalFileDuration) template.duration = parseFloat(optionalFileDuration);
    return template;
};
...
SkillsWriter.prototype.saveProcessingCard = function saveProcessingCard(optionalCallback) {
    const status = { code: SkillsErrorEnum.PENDING };
    const statusCard = this.createMetadataCard(cardType.STATUS, cardTitle.STATUS, null, null, status); // no entries or durations to pass
    return this.saveDataCards([statusCard], optionalCallback, skillStatus.PROCESSING);
};
...
SkillsWriter.prototype.saveErrorCard = function saveErrorCard(
    ...
    const failureType =
        optionalFailureType === skillStatus.TRANSIENT_FAILURE
            ? skillStatus.TRANSIENT_FAILURE
            : skillStatus.PERMANENT_FAILURE;
    const errorStatus = !optionalCustomErrorMessage
        ? { code: errorCode }
        : { code: 'custom_error', message: optionalCustomErrorMessage };
    const errorCard = this.createMetadataCard(cardType.STATUS, cardTitle.ERROR, null, null, errorStatus); // no entries or duration to pass
...
SkillsWriter.prototype.saveDataCards = function saveDataCards(
    ...
    const status = validateEnum(optionalStatus, skillStatus) ? optionalStatus : skillStatus.SUCCESS;
    let usage = null;
    // saving usage is allowed only if skillStatus type is success
    if (status === skillStatus.SUCCESS) {
        usage = validateUsage(optionalUsage) ? optionalUsage : DEFAULT_USAGE;
    }
    // create skill_invocations body
    const body = { status, usage };
    body.file = { type: 'file', id: this.fileId };
    body.metadata = { cards: listofDataCardJSONs };
